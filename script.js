...
function initializeApp() {
    ...
    participationModal.addEventListener('click', function(e) {
...
// FUNÇÃO MODIFICADA PARA CRIAR CARD DE RESULTADO COM NOVO BOTÃO
    ...
    card.innerHTML = `
        ...
        <div class="card-actions">
            ${peneira.status === 'aberta' ? `
                <button class="btn-primary" onclick="openParticipationModal(${peneira.id})">
...
// Função para abrir modal de participação
function openParticipationModal(peneiraId) {
    const peneira = peneirasData.find(p => p.id === peneiraId);
    if (!peneira) return;
    
    currentPeneiraModal = peneira;
    
    // Preencher informações do modal
    document.getElementById('modal-peneira-titulo').textContent = peneira.titulo;
    document.getElementById('modal-peneira-clube').textContent = peneira.clube;
    document.getElementById('modal-peneira-data').textContent = `${formatDate(peneira.data)} às ${peneira.horario}`;
    document.getElementById('modal-peneira-endereco').textContent = peneira.endereco || 'Endereço será definido';
    document.getElementById('modal-peneira-categoria').textContent = peneira.categoria;
    document.getElementById('modal-peneira-requisitos').textContent = peneira.requisitos;
    document.getElementById('modal-peneira-contato').textContent = peneira.contato;
    document.getElementById('modal-peneira-prazo').textContent = formatDate(peneira.prazoInscricao);
    
    // Preencher informações de vagas se a peneira estiver aberta
    if (peneira.status === 'aberta') {
        const vagasInfo = document.getElementById('modal-vagas-info');
        const vagasCount = document.getElementById('modal-vagas-count');
        const vagasProgress = document.getElementById('modal-vagas-progress');
        
        vagasInfo.style.display = 'block';
        vagasCount.textContent = `${peneira.vagasDisponiveis} de ${peneira.totalVagas} vagas`;
...
function closeParticipationModal() {
    ...
    currentPeneiraModal = null;
}

// Função para abrir direções a partir do modal
function openDirectionsFromModal() {
    if (currentPeneiraModal && currentPeneiraModal.endereco) {
        openDirections(currentPeneiraModal.endereco);
    }
}

// Função para entrar em contato a partir do modal
function contactClubFromModal() {
    if (currentPeneiraModal && currentPeneiraModal.contato) {
        const telefone = currentPeneiraModal.contato.replace(/\D/g, '');
        const whatsappUrl = `https://wa.me/55${telefone}?text=Olá! Gostaria de saber mais informações sobre a peneira "${currentPeneiraModal.titulo}" do ${currentPeneiraModal.clube}.`;
